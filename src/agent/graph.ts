...
- 编辑或写入代码文件后，建议调用 validate_file 校验该文件是否有编译/语法错误（环境缺少对应校验器时会自动跳过，不会报错）；若报错请据此修正。
...
/**
 * 计算一条 AI 消息的「动作签名」，用于死循环检测。
 * 取其全部 tool_calls 的 name+args 序列化；无工具调用时返回空串（不参与重复判定）。
 * 连续两次 agent 产出相同签名，说明模型在原地重复同一动作（典型死循环特征）。
 */
function actionSignature(ai: AIMessage): string {
  const calls = ai.tool_calls ?? [];
  if (calls.length === 0) return "";
  return JSON.stringify(calls.map((c) => ({ name: c.name, args: c.args })));
}

/**
 * Agent 图状态：
 * - messages：对话消息，用 messagesStateReducer 累积（能正确处理追加）。
 * - iterations：agent 节点已执行的轮数（用于首轮系统提示注入与可观测性，不再用于硬性截断）。
 * - lastSignature / repeats：死循环保护。repeats 记录「最近这个动作签名连续重复的次数」，
 *   签名变化即清零；只有连续重复达到阈值才判定为死循环并停止——正常推进的长任务不会被截断。
 */
export const AgentState = Annotation.Root({
  ...
  lastSignature: Annotation<string>({
    reducer: (_prev, next) => next,
    default: () => "",
  }),
  repeats: Annotation<number>({
    reducer: (_prev, next) => next,
    default: () => 0,
  }),
...
export interface AgentGraphDeps {
  ...
  /** 单轮提交内 agent 节点执行次数的绝对上限（防御性硬兜底） */
  maxIterations: number;
  /** 死循环检测阈值：连续重复同一动作达到该次数即停止 */
...
export function createAgentGraph(deps: AgentGraphDeps) {
  const { model, tools, manager, maxIterations, maxRepeatedActions } = deps;
  ...
  async function agentNode(
    ...
    // 死循环检测：与上一次动作签名相同则累加重复计数，否则清零（视为有进展）
    const sig = actionSignature(response as AIMessage);
    const repeats =
      sig !== "" && sig === state.lastSignature ? state.repeats + 1 : 1;
    return {
      messages: [response],
  ...
  function shouldContinue(state: typeof AgentState.State): "tools" | typeof END {
    ...
    // 死循环保护：同一动作连续重复达到阈值即停止；签名一变就清零，正常推进的长任务不受影响。
    if (state.repeats >= maxRepeatedActions) return END;
    // 绝对上限兜底：防御变化型 runaway（每步都不同、永不收敛）无限消耗。
    if (state.iterations >= maxIterations) return END;
    // 注意：以上两种强制停止都停在「带 tool_calls 的 AI」之后，会留下悬空 tool_use，
    // 由发送前的 repairToolPairs 补占位 tool_result，避免下一轮请求 400。
    return "tools";
  }
